...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(bb_frame.Min, bb_frame.Max, ImColor(0.01f, 0.01f, 0.01f, 1.f), window->WindowRounding, flags);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels);
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, GetColorU32(main_color), style.ScrollbarRounding);
...
bool ImGui::Keybind(const char* label, int* key)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(70, 50));
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, it_anim->second.mode ? main_color : ImVec4(0.6f, 0.6f, 0.6f, 1.f), GetIO().DeltaTime * anim_speed);
    ...
    {
        it_anim->second.key_size = ImLerp(it_anim->second.key_size, CalcTextSize(it_anim->second.key_name), g.IO.DeltaTime * anim_speed);
        if (*key < 250)
            it_anim->second.key_name = GetKeyName(ImGuiKey(*key));
        else
        {
            if (*key == 1001)
                it_anim->second.key_name = "L.Mouse";

            if (*key == 1002)
                it_anim->second.key_name = "R.Mouse";

            if (*key == 1003)
                it_anim->second.key_name = "M.Mouse";

            if (*key == 1004)
                it_anim->second.key_name = "x1.Mouse";

            if (*key == 1005)
                it_anim->second.key_name = "x2.Mouse";
        }
        PushFont(second_font);
        window->DrawList->AddText(total_bb.Min + ImVec2(65 - it_anim->second.key_size.x, 25 - CalcTextSize(it_anim->second.key_name).y / 2), GetColorU32(it_anim->second.text_color), it_anim->second.key_name);
        PopFont();
    }
    ...
    {
        PushFont(icon_font);
        it_anim->second.key_size = ImLerp(it_anim->second.key_size, CalcTextSize("6"), g.IO.DeltaTime * anim_speed);
        window->DrawList->AddText(total_bb.Min + ImVec2(55 - it_anim->second.key_size.x / 2, 25 - CalcTextSize("6").y / 2), GetColorU32(it_anim->second.text_color), "6");
        PopFont();
    }
    return pressed;
}

bool ImGui::SubTab(const char* label, int* tab, int number)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    window->DrawList->AddRectFilled(total_bb.Min - ImVec2(-10, 2), total_bb.Min + ImVec2(90, it_anim->second.line_offset), GetColorU32(main_color), 90.f, ImDrawFlags_RoundCornersBottom);
...
bool ImGui::Tab(const char* icon, int* tab, int number)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    window->DrawList->AddRectFilled(total_bb.Min - ImVec2(2, 0), total_bb.Min + ImVec2(it_anim->second.line_offset, 50.f), GetColorU32(main_color), 90.f, ImDrawFlags_RoundCornersRight);
...
bool ImGui::Checkbox(const char* label, bool* v, const char* hint)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.circle_offset = ImLerp(it_anim->second.circle_offset, *v ? 16.f : 0.f, g.IO.DeltaTime * anim_speed);
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, *v ? main_color : second_color, g.IO.DeltaTime * anim_speed);
    it_anim->second.circle_color = ImLerp(it_anim->second.circle_color, *v ? ImColor(1.f, 1.f, 1.f, 1.f) : ImColor(0.6f, 0.6f, 0.6f, 1.f), g.IO.DeltaTime * anim_speed);
    it_anim->second.text_color_offset = ImLerp(it_anim->second.text_color_offset, *v ? ImColor(0.f, 0.f, 0.f, 0.0f) : ImColor(0.f, 0.f, 0.f, 0.4f), g.IO.DeltaTime * anim_speed);

    window->DrawList->AddRect(total_bb.Min, total_bb.Max, background_color, style.FrameRounding);
    window->DrawList->AddRect(total_bb.Max - ImVec2(60, 44), total_bb.Max - ImVec2(20, 22), GetColorU32(it_anim->second.rect_color), 25, 0, 1.5f);
    window->DrawList->AddCircleFilled(total_bb.Max - ImVec2(48 - it_anim->second.circle_offset, 33), 7.f, GetColorU32(it_anim->second.circle_color), 60);
    ...
    window->DrawList->AddText(hint_font, 16, total_bb.Min + ImVec2(15, 25), ImColor(0.6f, 0.6f, 0.6f, 1.f) - GetColorU32(it_anim->second.text_color_offset), hint);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int value, const char* hint)
{
    ...
    const ImRect rect_bb(total_bb.Max - ImVec2(80 + it_anim->second.text_size.x, 46), total_bb.Max - ImVec2(20, 23));
    ItemSize(total_bb, style.FramePadding.y);
    ItemAdd(total_bb, id);
    ...
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? value * 30 + 1 : 0.f, g.IO.DeltaTime * 12.f);

    it_anim->second.icon_color = ImLerp(it_anim->second.icon_color, it_anim->second.opened_combo ? main_color : ImVec4(0.6f, 0.6f, 0.6f, 1.f), g.IO.DeltaTime * 12.f);

    window->DrawList->AddRect(total_bb.Min, total_bb.Max, background_color, style.FrameRounding);

    window->DrawList->AddRect(rect_bb.Min, rect_bb.Max, GetColorU32(second_color), style.FrameRounding);

    window->DrawList->AddRectFilled(rect_bb.Max - ImVec2(35, 20), rect_bb.Max - ImVec2(3, 3), GetColorU32(second_color), style.FrameRounding);

    window->DrawList->AddText(total_bb.Min + ImVec2(15, 5), ImColor(1.f, 1.f, 1.f, 1.f), label);

    window->DrawList->AddText(hint_font, 16, total_bb.Min + ImVec2(15, 25), ImColor(0.6f, 0.6f, 0.6f, 1.f), hint);

    window->DrawList->AddText(icon_font, 16, rect_bb.Max - ImVec2(28, 20), GetColorU32(it_anim->second.icon_color), "0");

    PushFont(second_font);
    window->DrawList->AddText(rect_bb.Min + ImVec2(10, (rect_bb.Max.y - rect_bb.Min.y) / 2 - CalcTextSize(preview_value).y / 2), ImColor(0.8f, 0.8f, 0.8f, 1.f), preview_value);
    it_anim->second.text_size = ImLerp(it_anim->second.text_size, CalcTextSize(preview_value), g.IO.DeltaTime * anim_speed);
    PopFont();
...
void ImGui::MultiCombo(const char* label, bool combos[], const char* items[], int items_count, const char* hint)
{
    ...
    if (BeginCombo(label, preview.c_str(), 0, items_count, hint))
    {
...
struct slider_state {
    ...
    ImVec4 grab_color;
    float grab_offset;
    float value_buf_width;
};

bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, const char* hint)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImRect slider_bb(frame_bb.Min + ImVec2(300, 30), frame_bb.Max - ImVec2(20, 30));
    ...
    it_anim->second.grab_offset = ImLerp(it_anim->second.grab_offset, (grab_bb.Min.x + (grab_bb.Max.x - grab_bb.Min.x) / 2) - slider_bb.Min.x, g.IO.DeltaTime * anim_speed);

    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, background_color, style.FrameRounding);

    window->DrawList->AddRectFilled(slider_bb.Min, slider_bb.Max, GetColorU32(second_color), 25);
    window->DrawList->AddRectFilled(slider_bb.Min, slider_bb.Min + ImVec2(it_anim->second.grab_offset, slider_bb.Max.y - slider_bb.Min.y), GetColorU32(main_color), 25);

    window->DrawList->AddShadowCircle(slider_bb.Min + ImVec2(it_anim->second.grab_offset, (slider_bb.Max.y - slider_bb.Min.y) / 2), 8.f, ImColor(0.f, 0.f, 0.f, 1.f), 15.f, ImVec2(0, 0));
    window->DrawList->AddCircleFilled(slider_bb.Min + ImVec2(it_anim->second.grab_offset, (slider_bb.Max.y - slider_bb.Min.y) / 2), 6.f, GetColorU32(main_color), 60);

    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);

    window->DrawList->AddText(ImVec2(frame_bb.Min.x + 15.f, frame_bb.Min.y + 15 - CalcTextSize(label).y / 2), ImColor(1.f, 1.f, 1.f, 1.f), label);

    PushFont(second_font);
    window->DrawList->AddText(ImVec2(frame_bb.Max.x - (20 + CalcTextSize(value_buf).x), frame_bb.Min.y + 15 - CalcTextSize(value_buf).y / 2), ImColor(1.f, 1.f, 1.f, 1.f), value_buf);
    PopFont();

    window->DrawList->AddText(hint_font, 16, frame_bb.Min + ImVec2(15, 25), ImColor(0.6f, 0.6f, 0.6f, 1.f), hint);
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags, const char* hint)
{
    return SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags, hint);
}

bool ImGui::SliderFloat2(const char* label, float v[2], float v_min, float v_max, const char* format, ImGuiSliderFlags flags, const char* hint)
{
    return SliderScalarN(label, ImGuiDataType_Float, v, 2, &v_min, &v_max, format, flags);
}

bool ImGui::SliderFloat3(const char* label, float v[3], float v_min, float v_max, const char* format, ImGuiSliderFlags flags, const char* hint)
{
    return SliderScalarN(label, ImGuiDataType_Float, v, 3, &v_min, &v_max, format, flags);
}

bool ImGui::SliderFloat4(const char* label, float v[4], float v_min, float v_max, const char* format, ImGuiSliderFlags flags, const char* hint)
{
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags, const char* hint)
{
    return SliderScalar(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags, hint);
}

bool ImGui::SliderInt2(const char* label, int v[2], int v_min, int v_max, const char* format, ImGuiSliderFlags flags, const char* hint)
{
    return SliderScalarN(label, ImGuiDataType_S32, v, 2, &v_min, &v_max, format, flags);
}

bool ImGui::SliderInt3(const char* label, int v[3], int v_min, int v_max, const char* format, ImGuiSliderFlags flags, const char* hint)
{
    return SliderScalarN(label, ImGuiDataType_S32, v, 3, &v_min, &v_max, format, flags);
}

bool ImGui::SliderInt4(const char* label, int v[4], int v_min, int v_max, const char* format, ImGuiSliderFlags flags, const char* hint)
{
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    it_anim->second.shadow_thinkess = ImLerp(it_anim->second.shadow_thinkess, render_cursor ? 25.f : 15.f, g.IO.DeltaTime * 6.f);

    if (!is_multiline)
    {
        RenderNavHighlight(frame_bb, id);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(second_color), 4.f, 0, 1.f);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            ...
            ImRect cursor_screen_rect(cursor_screen_pos.x, cursor_screen_pos.y - g.FontSize + 7.5f, cursor_screen_pos.x + 1.0f, cursor_screen_pos.y + 3.5f);
            ...
            draw_window->DrawList->AddLine(it_anim->second.slow_moveX0, it_anim->second.slow_moveX1, ImColor(1.f, 1.f, 1.f, it_anim->second.alpha_line));
    ...
    {
        PushFont(second_font);
        window->DrawList->AddText(frame_bb.Min + ImVec2(15.f, (frame_bb.Max.y - frame_bb.Min.y) / 2 - CalcTextSize(label).y / 2), ImColor(0.5f, 0.5f, 0.5f, 1.f), label);
        PopFont();
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImVec2 frame_size = ImVec2(250, 30);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleColor(ImGuiCol_PopupBg, ImVec4(0.05f, 0.05f, 0.05f, 1.f));
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                it_anim->second.picker_size = ImLerp(it_anim->second.picker_size, 10.f, g.IO.DeltaTime * anim_speed);
                picker_active_window = g.CurrentWindow;
                ImGuiColorEditFlags picker_flags_to_forward = ImGuiColorEditFlags_DataTypeMask_ | ImGuiColorEditFlags_PickerMask_ | ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaBar;
                ImGuiColorEditFlags picker_flags = (flags_untouched & picker_flags_to_forward) | ImGuiColorEditFlags_DisplayMask_ | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_AlphaPreviewHalf;
                SetNextItemWidth(square_sz * it_anim->second.picker_size); // Use 256 + bar sizes?
            ...
            it_anim->second.picker_size = ImLerp(it_anim->second.picker_size, 0.f, g.IO.DeltaTime * anim_speed);
        PopStyleColor();
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    ImVec2 bar_pos = window->DC.CursorPos + ImVec2(10, 118);
    float square_sz = GetFrameHeight();
    float bars_width = 190.f; // Arbitrary smallish width of Hue/Alpha picking bars
    float sv_picker_size = 190; // Saturation/Value picking box
    float sv_bar_size = 20; // Saturation/Value picking box
    float bar0_pos_x = GetWindowPos().x + 10;
    ...
    SetCursorScreenPos(ImVec2(bar1_pos_x + 10, bar_pos.y + 30));
    InvisibleButton("add_size", ImVec2(bars_width, 1));
    ...
    // Render SV Square
    draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), col_white, hue_color32, hue_color32, col_white);
    draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), 0, 0, col_black, col_black);
    ...
    draw_list->AddRect(picker_pos, picker_pos + ImVec2(sv_picker_size, 125), ImColor(0.05f, 0.05f, 0.05f, 1.f), 6.f, 0, 8.f);

    draw_list->AddRectFilled(ImVec2(it_anim->second.hue_bar + bar_pos.x - 5.f, bar_pos.y + 2.f), ImVec2(it_anim->second.hue_bar + bar_pos.x + 1.f, bar_pos.y + sv_bar_size - 4.f), ImColor(1.f, 1.f, 1.f, 1.f), 5.f);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    else
    {
        ...
        window->DrawList->AddText(icon_font, 26, bb_inner.Min, GetColorU32(col_source), "5");
    }
    RenderNavHighlight(bb, id);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const ImVec2 text_min = pos + ImVec2(10, 0);
    ...
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, selected ? ImColor(0.3f, 0.3f, 0.3f, 0.5f) : ImColor(0.1f, 0.1f, 0.1f, 0.5f), g.IO.DeltaTime * anim_speed);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, selected ? main_color : ImVec4(0.9f, 0.9f, 0.9f, 1.f), g.IO.DeltaTime * anim_speed);
    it_anim->second.text_offset = ImLerp(it_anim->second.text_offset, selected ? 8.f : 0.f, g.IO.DeltaTime * anim_speed);
    ...
    window->DrawList->AddRectFilled(bb.Min + ImVec2(0, 5.f), bb.Min + ImVec2(it_anim->second.text_offset, 25), GetColorU32(main_color), 5.f, ImDrawFlags_RoundCornersRight);

    PushFont(second_font);
    window->DrawList->AddText(bb.Min + ImVec2(10 + it_anim->second.text_offset, (bb.Max.y - bb.Min.y) / 2 - CalcTextSize(label).y / 2), GetColorU32(it_anim->second.text_color), label);
    PopFont();
